...
var Verify = (function () {
  ...
  var REQUIRED_LISTS = [
    'BUDGET_TYPE', 'SUB_TYPE', 'METHOD', 'ACTIVITY_TYPE', 'CHANNEL', 'EXCEPTION_REASON',
    'UNIT', 'DEPARTMENT', 'RESPONSE_STATUS', 'QUALIFICATION_STATUS', 'VENDOR_STATUS',
    'EXCEPTION_STATUS', 'REF_TYPE', 'ROLE'
  ];
  ...
  var SAMPLE_CODES = {
    ...
    UNIT: ['PCS', 'SET', 'SQM', 'JOB'],
    MEDIA_TYPE: ['BILLBOARD', 'LED', 'SIGNAGE']
  };

  var M1_TRANSITIONS = {
    INTAKE: ['SOURCING', 'CANCELLED'],
    SOURCING: ['SOURCING_DONE', 'INTAKE', 'CANCELLED'],
    SOURCING_DONE: ['SOURCING', 'CLOSED', 'CANCELLED']
  };
  ...
  function run() {
    ...
    check(checks, 'settings', 'ค่าตั้งค่าหลัก', function () {
      var problems = [];
      var minQuotes = Config.getNumber('MIN_QUOTES', 0);
      if (!(minQuotes > 0)) problems.push('MIN_QUOTES ต้องเป็นตัวเลขมากกว่า 0 (ตอนนี้ "' +
        Config.get('MIN_QUOTES', '') + '")');
      ...
      return pass('ใบเสนอราคาขั้นต่ำ ' + minQuotes + ' ราย · โซนเวลา ' + Config.getTimezone() +
        ' · เตือนเวลา ' + reminderHour + ':00');
    });
    ...
    check(checks, 'statuses', 'สถานะงานและเส้นทางของ Module 1', function () {
      var all = Config.getStatusMaster();
      if (all.length < 10) return fail('มีสถานะเพียง ' + all.length + ' รายการ จากที่ควรมี 10 — กรุณารัน setup()');
      var problems = [];
      Object.keys(M1_TRANSITIONS).forEach(function (code) {
        var status = Config.getStatus(code);
        if (!status) { problems.push('ไม่มีสถานะ ' + code); return; }
        M1_TRANSITIONS[code].forEach(function (next) {
      ...
      return pass(all.length + ' สถานะ และเส้นทางของ Module 1 ครบ');
    });
    ...
    check(checks, 'trigger', 'อีเมลแจ้งเตือนรายวัน', function () {
      ...
      if (installed.length === 0) {
        return warn('ยังไม่ได้ติดตั้ง trigger — กรุณารัน installTriggers() ' +
          '(ไม่มี trigger = ไม่มีอีเมลเตือน และใบเสนอราคาที่หมดอายุเองจะไม่ถูกตรวจพบ)');
      }
