...
var Notification = (function () {
  ...
  function onAutoRevert(caseRecord, from, to, message) {
    send(caseRecord.Buyer_Owner, '[' + caseRecord.Case_ID + '] ระบบย้อนสถานะงานอัตโนมัติ', [
      ...
      '<p>กรุณาตรวจสอบและดำเนินการให้ครบเงื่อนไขก่อนปิดขั้นตอนหา Vendor อีกครั้ง</p>'
    ]);
  }

  function onExceptionRequested(caseRecord, user) {
    send(headEmails(), '[' + caseRecord.Case_ID + '] มีคำขอยกเว้นจำนวนใบเสนอราคารออนุมัติ', [
      '<p>' + escape(user.name) + ' ขอยกเว้นจำนวนใบเสนอราคาขั้นต่ำ</p>',
      caseLink(caseRecord),
      '<p>เหตุผล: <strong>' + escape(Config.getList('EXCEPTION_REASON').filter(function (r) {
        return r.code === caseRecord.Exception_Reason_Code;
      }).map(function (r) { return r.label; })[0] || caseRecord.Exception_Reason_Code) + '</strong></p>',
      '<p>คำอธิบาย: ' + escape(caseRecord.Exception_Note) + '</p>'
    ]);
  }

  function onExceptionDecided(caseRecord, user, approved, note) {
    send(caseRecord.Buyer_Owner,
      '[' + caseRecord.Case_ID + '] คำขอยกเว้น' + (approved ? 'ได้รับอนุมัติ' : 'ไม่ได้รับอนุมัติ'), [
        '<p>' + escape(user.name) + (approved ? ' อนุมัติ' : ' ไม่อนุมัติ') + 'คำขอยกเว้นของคุณ</p>',
        caseLink(caseRecord),
        '<p>ความเห็น: ' + escape(note) + '</p>'
      ]);
  }
  ...
  /**
   * SPEC §6.1 — a quotation expires with the passage of time, not with a write,
   * so the daily job is what catches it.
   */
  function recheckOpenCases() {
    var reverted = 0;
    Repository.query('Cases', {
      where: function (c) { return c.Status === 'SOURCING_DONE'; }
  ...
  return {
    ...
    onExceptionRequested: onExceptionRequested,
    onExceptionDecided: onExceptionDecided,
