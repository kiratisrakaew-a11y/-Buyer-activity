...
var ActivityService = (function () {
  ...
  /** An activity about a vendor must be about a vendor actually invited to this Case. */
  function assertVendorOnCase(caseId, vendorId) {
    if (Utils.isBlank(vendorId)) return;
    var invited = Repository.queryByCase('Case_Vendors', caseId).some(function (cv) {
      return cv.Vendor_ID === vendorId;
    });
    if (!invited) {
      throw Err.validation('ผู้ขายรายนี้ไม่ได้อยู่ในงาน ' + caseId, { field: 'Vendor_ID' });
    }
  }
