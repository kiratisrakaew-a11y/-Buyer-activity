...
รันแล้วเปิด Execution log จะได้ checklist 13 ข้อ
...
...
สรุป: ผ่าน 13 · เตือน 0 · ไม่ผ่าน 0
พร้อมใช้งานจริง
================================
```
...
| **ไม่มีอีเมลเตือน**รายวัน | ยังไม่ได้รัน `installTriggers()` | รัน แล้วตรวจด้วย `verifyDeployment()` ข้อ 11 |
...
| เพิ่มผู้ขายแล้วขึ้นว่า **"ถูกเพิ่มในงานนี้อยู่แล้ว"** ทั้งที่ไม่มี | มีสองแถวในชีต `Vendors` ถือ `Vendor_ID` ซ้ำกัน | รัน `verifyDeployment()` ดูข้อ 3 จะบอกว่ารหัสไหนซ้ำ แล้วทำตามหัวข้อ 9 |
| ขึ้นว่า **"ข้อมูลขัดแย้ง: รหัส ... มีอยู่ N แถว"** | รหัสซ้ำตามข้างต้น | ทำตามหัวข้อ 9 |

ดูรายละเอียด error ได้ที่ `Executions` ในหน้า Apps Script (เมนูซ้ายมือ รูปนาฬิกา)

---

## 9. แก้รหัสซ้ำ (เมื่อ `verifyDeployment()` ข้อ 3 ขึ้น "ไม่ผ่าน")

ระบบเคยมีบั๊กที่ตัวนับแจกรหัสซ้ำเมื่อกดสร้างติดๆ กัน (ขาด `SpreadsheetApp.flush()`)
โค้ดแก้แล้ว แต่**แถวที่เขียนไปตอนนั้นยังค้างอยู่**

อาการที่เจอ: เพิ่มผู้ขายแล้วบอกว่ามีอยู่แล้วทั้งที่ไม่มี · เห็นชื่อผิดคน · แก้แล้วไปเปลี่ยนแถวอื่น

### ขั้นตอนแก้

1. รัน `verifyDeployment()` — ข้อ 3 จะบอกว่า**ชีตไหน รหัสไหน**ซ้ำ
2. เปิดชีตนั้น หาแถวที่ใช้รหัสซ้ำกัน (สูตรช่วย: `=FILTER(A2:C, COUNTIF(A2:A, A2:A)>1)`)
3. ดูว่า**แถวไหนถูกอ้างอิงอยู่จริง** — เช่น `Vendor_ID` ในชีต `Case_Vendors`
4. แถวที่ถูกอ้างอิง — **อย่าแตะ** · แถวที่ไม่มีใครอ้างถึง — แก้รหัสเป็นเลขใหม่ที่ยังไม่มีใครใช้
5. ไปชีต `Counters` ตั้ง `Last_No` ของตารางนั้นให้**สูงกว่าเลขที่มากที่สุดที่ใช้อยู่** ไม่งั้นจะซ้ำอีก
6. รัน `verifyDeployment()` อีกครั้ง ข้อ 3 ต้องขึ้น "ผ่าน"

> ⚠️ **ห้ามลบแถวทิ้ง** ถ้ามีตารางอื่นอ้างรหัสนั้นอยู่ — จะทำให้หน้างานโชว์รหัสดิบแทนชื่อ
> ระบบใช้การลบแบบ soft delete (ตั้ง `Is_Deleted` = TRUE) ไม่เคยลบแถวจริง
